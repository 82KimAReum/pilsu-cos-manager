            ...
            <div class="row">
                   ...
                   <br/>
                            <table class="table table-hover" >    
                                    <caption>
                                        <a href='/biz/faq/1'>전체보기</a> (<span id="faqCount">${page.count }</span>) 
                                        
                                    </caption>              
                                <thead>                  
                                    <tr class="">
                                        <th style="width:5%"><input type="checkbox" name="chkAll"/></th>
                                        <th style="width:10%">번호</th><th >제목</th>
                                    </tr>
                                </thead>
                                <tbody id="result">
                                	<c:forEach items="${data }" var="faq" varStatus="status">
                                		<tr>
                                			<td><input type="checkbox" name="chk" value="${faq.faqno }"/></td>
                                			<td>${(page.page-1)*10+status.count }</td>
                                			<td><a href='/biz/faq/detail/${faq.faqno }'>${faq.title }</a></td>
                                		</tr>
                                	</c:forEach>
                                </tbody>
                            ...
                            <table class="table table-hover" >
                                    <tr>
                                        ...
                                        <td><button id="btnWrite" style="float: right">글쓰기</button></td>
                                    </tr>
                            </table>
                     	...
                     	<ul class="pagination" >
		                    <c:choose>
			                     <c:when test="${page.page gt 1 }" >
			                     	<li><a href='/biz/faq/${page.page-1 }${searchData}'>left</a></li>
			                     </c:when>
			                     <c:otherwise >
			                     	<li><a href=''>left</a></li>
			                     </c:otherwise>
		                     ...
		                     <c:forEach begin="${page.pageStart }" end="${page.pageEnd }"  varStatus="status">
		                     	<li <c:out value="${status.current ==page.page?'class=active':'' }" /> ><a href='/biz/faq/${status.current }${searchData}'>${status.current }</a></li>
		                     </c:forEach>
		                     <c:choose>
			                     ...
			                     <c:otherwise >
			                     	<li><a href='/biz/faq/${page.page+1 }${searchData}'>right</a></li>
			                     </c:otherwise>
		                     </c:choose>
                     </ul>
           			</div>
		...
		var text="";
		 ...
		 function searchFaq(page){
			 text=$("#txtSearch").val();
	       	...
	       	self.location="/biz/faq/1?data="+text;
		}
