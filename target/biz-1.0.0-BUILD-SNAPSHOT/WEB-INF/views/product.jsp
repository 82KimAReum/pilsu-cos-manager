            ...
            <div class="row">
                   ...
                   <br/>
                            <table class="table table-hover" >    
                                    <caption>
                                       <a href='/biz/product/1'>전체보기</a> (<span >${page.count }</span>)
                                ...
                                <tbody id="result">
                                	<c:forEach items="${data }" var="product" varStatus="status">
                                		<tr>
                                			<td><input type="checkbox" name="chk" value="${product.productcode }"/></td>
                                			<%-- <td>${(page.page-1)*10+status.count }</td> --%>
                                			<td><a href='/biz/product/detail/${product.productcode}'>${product.productcode}</a></td>
                                			<td><a href='/biz/product/detail/${product.productcode}'>${product.productname }</a></td>
                            ...
                            <table class="table table-hover" >
                                    <tr>
                                        ...
                                        <td><button  id="btnWrite" style="float: right">상품추가</button></td>
                                    </tr>
                            </table>
                     	...
                     	<ul class="pagination" >
		                    <c:choose>
			                     <c:when test="${page.page gt 1 }" >
			                     	<li><a href='/biz/product/${page.page-1 }${searchData}'>left</a></li>
			                     </c:when>
			                     <c:otherwise >
			                     	<li><a href=''>left</a></li>
			                     </c:otherwise>
		                     ...
		                     <c:forEach begin="${page.pageStart }" end="${page.pageEnd }"  varStatus="status">
		                     	<li <c:out value="${status.current ==page.page?'class=active':'' }" /> ><a href='/biz/product/${status.current }${searchData}'>${status.current }</a></li>
		                     </c:forEach>
		                     <c:choose>
			                     ...
			                     <c:otherwise >
			                     	<li><a href='/biz/product/${page.page+1 }${searchData}'>right</a></li>
			                     </c:otherwise>
		                     </c:choose>
                     </ul>
           			</div>
            ...
            $(function(){
                ...
                ///////////////////////상품추가//////////////////////
         		$("#btnWrite").bind("click",function(){
         			location.href="/biz/productWrite";
         		});
        ...
        $("#btnDelete").bind("click",function(){
        	 var r = confirm("정말 삭제하시겠습니까?");
        	    if (r == true) {
					...
					location.replace("/biz/product/"+path); 
        	    } else {
        	        return false;
        	    }
		...
		var text="";
		 ...
		 function searchFaq(page){
			 text=$("#txtSearch").val();
	       	...
	       	self.location="/biz/product/1?data="+text;
		}
