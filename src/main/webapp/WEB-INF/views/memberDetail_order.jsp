				 ...
				 <caption>주문자 정보</caption> 
                    <tbody>
						<tr style="text-align: center;">
							...
							<td><button type="button" class="btn btn-default" id="order" disabled="disabled">주문 목록</button></td>
							...
							<th>주문번호</th>
                               ...
                               <tbody id="result">
                                	<c:forEach items="${data }" var="order" varStatus="status">
                                		<tr>
                                			<%-- <td><input type="checkbox" name="chk" value="${order.orderno }"/></td> --%>
                     	...
                     	<ul class="pagination" >
		                    <c:choose>
			                     <c:when test="${page.page gt 1 }" >
			                     	<li><a href='/biz/member/detail/${email }/order/${page.page-1}'>left</a></li>
			                     </c:when>
			                     <c:otherwise >
			                     	<li><a href=''>left</a></li>
			                     </c:otherwise>
		                     ...
		                     <c:forEach begin="${page.pageStart }" end="${page.pageEnd }"  varStatus="status">
		                     	<li <c:out value="${status.current ==page.page?'class=active':'' }" /> ><a href='/biz/member/detail/${email }/order/${status.current }'>${status.current }</a></li>
		                     </c:forEach>
		                     <c:choose>
			                     ...
			                     <c:otherwise >
			                     	<li><a href='/biz/member/detail/${email }/order/${page.page+1}'>right</a></li>
			                     </c:otherwise>
		                     </c:choose>
                     </ul>
           			</div>
	   ...
	   var r = confirm("정말 삭제하시겠습니까?");
	    if (r == true) {
        	...
        	str=str.substr(0,str.length-1);
				 ...
				 location.replace("/biz/member/detail/${email }/order/${page.page}"); 
	    } else {
